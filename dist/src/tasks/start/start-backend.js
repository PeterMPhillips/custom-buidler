...
async function startBackend({ appName, silent }, bre) {
    ...
    const dao = await create_dao_1.createDao(bre.web3, bre.artifacts, daoFactoryAddress);
    ...
    if (hooks && hooks.setupPermissions) {
        ...
        async function createPermission(address, permission) {
            await acl.createPermission(address, proxy.address, permission, rootAccount, { from: rootAccount });
        }
        await hooks.setupPermissions({ dao, proxy, createPermission, log: logger_1.logHook('setupPermissions') }, bre);
    }
    else {
        await set_permissions_1.setAllPermissionsOpenly(dao, proxy, arapp, bre.web3, bre.artifacts);
